...
for this_performer in performaer_list:
    ...
    for item_path in audio_path_list:
        ...
        save_out = np.concatenate((piano_roll_dictionary[this_performer][name_code], mfcc_dictionary[this_performer][name_code]), axis=1)
        print(save_out.shape)
        # raise()
        # Save as pickle
        if not os.path.exists('./preprocessed_data_save_both/all/'):
            os.makedirs('./preprocessed_data_save_both/all/')
        
        combine_data_output = open('./preprocessed_data_save_both/all/' +
        ...
        if not os.path.exists('./preprocessed_data_save_both/motion/'):
            os.makedirs('./preprocessed_data_save_both/motion/')
            
        motion_data_output = open('./preprocessed_data_save_both/motion/' +
                                  name_code + '_motion_data.pkl', 'wb')
